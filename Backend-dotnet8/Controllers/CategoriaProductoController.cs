...
namespace Backend_dotnet8.Controllers
{
    ...
    {
        private readonly IGenericoService<CategoriaProducto> _service;
        public CategoriaProductoController(IGenericoService<CategoriaProducto> service)
        {
            _service = service;
        }
        [HttpPost]
        [Route("Crear")]
        public async Task<Retorno<bool>> Create([FromBody] CategoriaProductoEntrada valor)
        {

            try
            {
                CategoriaProducto entidadMapeada = Mapping.GetMapper(valor);
        ...
        public async Task<Retorno<bool>> Delete(Guid id)
        {
            ...
            try
            {
                CategoriaProducto? entidadMapeada = await _service.GetByIdAsync(id);
                if (entidadMapeada != null)
                {
