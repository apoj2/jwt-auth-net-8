...
namespace Backend_dotnet8.Controllers
{
    ...
    {
        private readonly IGenericoService<Producto> _service;
        public ProductoController(IGenericoService<Producto> service)
        {
            _service = service;
        }
        [HttpPost]
        [Route("Crear")]
        public async Task<Retorno<bool>> Create([FromBody] ProductoEntrada valor)
        {

            try
            {
                Producto entidadMapeada = Mapping.GetMapper(valor);
        ...
        public async Task<Retorno<bool>> Delete(Guid id)
        {
            ...
            try
            {
                Producto? entidadMapeada = await _service.GetByIdAsync(id);
                if (entidadMapeada != null)
                {
