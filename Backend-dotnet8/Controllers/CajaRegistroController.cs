...
namespace Backend_dotnet8.Controllers
{
    ...
    [ApiController]
    public class CajaRegistroController : ControllerBase
    {
        private readonly IGenericoService<CajaRegistro> _service;
        public CajaRegistroController(IGenericoService<CajaRegistro> service)
        {
            _service = service;
        }
        [HttpPost]
        [Route("Crear")]
        public async Task<Retorno<bool>> Create([FromBody] CajaRegistroEntrada valor)
        {

            try
            {
                CajaRegistro entidadMapeada = Mapping.GetMapper(valor);
        ...
        public async Task<Retorno<bool>> Delete(Guid id)
        {
            ...
            try
            {
                CajaRegistro? entidadMapeada = await _service.GetByIdAsync(id);
                if (entidadMapeada != null)
                {
